...
class Model(nn.Module):
    def __init__(self, options):
        ...
        # dsc
        self.segmentation_pred = nn.Conv2d(512, NUM_CORNERS + NUM_ICONS + 2 + NUM_ROOMS + 2 + NUM_SLANTS, kernel_size=1)
    ...
    def forward(self, inp):
        ...
        return torch.sigmoid(segmentation[:, :, :, :NUM_CORNERS]), segmentation[:, :, :, NUM_CORNERS:NUM_CORNERS + NUM_ICONS + 2], segmentation[:, :, :, NUM_CORNERS + NUM_ICONS + 2:-NUM_SLANTS], torch.sigmoid(segmentation[:, :, :, -NUM_SLANTS:])
